...
class GameState:
    ...
    # UI update flags
    ui_needs_update: Dict[str, bool] = field(default_factory=lambda: {
        "character": False,
        "map": False,
        "inventory": False,
        "combat_log": False,
...
class GameStateManager:
    ...
    def new_game(self, character_name: str = "Hero") -> None:
        ...
        self.state.character._combat_log = self.state.combat_log
    ...
    def _update_character_panel(self) -> None:
        ...
        panel = self.main_window.character_panel
        if not panel or not self.state.character:
            return
    ...
    def _update_map_widget(self) -> None:
        ...
        widget = self.main_window.map_widget
        if not widget or not self.state.character or not self.state.current_floor:
            return
    ...
    def _update_combat_log(self) -> None:
        ...
        panel = self.main_window.info_panel
        if not panel or not self.state.combat_log:
            return
    ...
    def _update_floor_info(self) -> None:
        ...
        panel = self.main_window.info_panel
        if not panel or not self.state.current_floor:
            return
