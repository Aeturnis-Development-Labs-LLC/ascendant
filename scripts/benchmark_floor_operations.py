...
def benchmark_operation(
    operation_name: str, operation, iterations: int = 100
...
def main():
    ...
    print(
        f"| Floor Generation | {min(gen_times):.2f} | {gen_overall_avg:.2f} | {max(gen_times):.2f} | <100ms | {gen_status} |"
    )
    ...
    print(
        f"| Room Connection | {min(conn_times):.2f} | {conn_overall_avg:.2f} | {max(conn_times):.2f} | <50ms | {conn_status} |"
    )
    ...
    print(
        f"| Pathfinding Check | {min(path_times):.2f} | {path_overall_avg:.2f} | {max(path_times):.2f} | <50ms | {path_status} |"
    )
    ...
    print(
        f"| Full Floor Creation | {min(full_times):.2f} | {full_overall_avg:.2f} | {max(full_times):.2f} | <500ms | {full_status} |"
    )
    ...
    for i, seed in enumerate(seeds):
        print(
            f"| {seed} | {gen_times[i]:.2f}ms | {conn_times[i]:.2f}ms | {path_times[i]:.2f}ms | {full_times[i]:.2f}ms |"
        )
